---
id: moros
nombre: Moros
rol: generico
edad: 28
sexo: M
is_alive: True
posicion: [45, 30]
humor: 0.859
energia: 0.665
ansiedad: 0.224
needs:
  hambre: 0.084
  fatiga: 0.335
  sed: 0.075
  sociabilidad: 0.020
arquetipo_dominante: sombra
estado_conductual: aislamiento
arquetipos:
  self: 0.524
  persona: 0.500
  sombra: 0.841
  anima_animus: 0.400
  heroe: 0.218
  sabio: 0.400
  trickster: 0.500
  madre: 0.403
  padre: 0.400
  nino_divino: 0.300
  gobernante: 0.400
  rebelde: 0.300
---
# 👤 Persona: Moros

> **Rol:** `generico` | **Edad:** 28 | **Sexo:** M | **Ubicación:** `(45, 30)`
> **Estado Vital:** **🟢 Conectado / Vivo**

---

## 📊 Estado Psicobiológico

### Capa Emocional
| Métrica | Visual | Valor |
| :--- | :---: | :---: |
| **Humor** | `[▓▓▓▓▓▓▓▓▓░]` | 0.86 |
| **Energía** | `[▓▓▓▓▓▓▓░░░]` | 0.66 |
| **Ansiedad** | `[▓▓░░░░░░░░]` | 0.22 |

### Necesidades Biológicas
| Necesidad | Visual | Valor | Estado |
| :--- | :---: | :---: | :--- |
| **Hambre** | `[▓░░░░░░░░░]` | 0.08 | 🟢 Saciado |
| **Sed** | `[▓░░░░░░░░░]` | 0.08 | 🟢 Hidratado |
| **Fatiga** | `[▓▓▓░░░░░░░]` | 0.34 | 🟢 Descansado |
| **Sociabilidad** | `[░░░░░░░░░░]` | 0.02 | 🟢 Conectado |

---

## 🧠 Perfil Psicológico

### Capa Jungiana: Vector de Arquetipos
| Arquetipo | Fuerza | Barra Visual |
| :--- | :---: | :--- |
| **Self** | `0.524` | `▓▓▓▓▓▓░░░░░░` |
| **Persona** | `0.500` | `▓▓▓▓▓▓░░░░░░` |
| **Sombra** | `0.841` | `▓▓▓▓▓▓▓▓▓▓░░` |
| **Anima animus** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Heroe** | `0.218` | `▓▓▓░░░░░░░░░` |
| **Sabio** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Trickster** | `0.500` | `▓▓▓▓▓▓░░░░░░` |
| **Madre** | `0.403` | `▓▓▓▓▓░░░░░░░` |
| **Padre** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Nino divino** | `0.300` | `▓▓▓▓░░░░░░░░` |
| **Gobernante** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Rebelde** | `0.300` | `▓▓▓▓░░░░░░░░` |

### Complejos Activos
| Complejo | Intensidad | Barra Visual | Estado |
| :--- | :---: | :--- | :---: |
| Abandono | `0.65` | `▓▓▓▓▓▓░░░░` | 💤 Inactivo |
| Inferioridad | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Poder | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Culpa | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Materno | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Trascendencia | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |

---

## 👥 Red Social e Interacciones

### Vínculos Emocionales
| Agente Relacionado | Vínculo | Visual Vínculo | Entrelazado |
| :--- | :---: | :---: | :---: |
| [[arete]] | `+0.08` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[bios]] | `+0.16` | `▓▓▓▓▓▓░░░░` | Ninguno |
| [[chara]] | `+0.08` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[elpis]] | `+0.08` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[iris]] | `+0.14` | `▓▓▓▓▓▓░░░░` | Ninguno |
| [[kairos]] | `+0.08` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[kore]] | `-1.00` | `░░░░░░░░░░` | Ninguno |
| [[sophron]] | `+0.08` | `▓▓▓▓▓░░░░░` | Ninguno |

---

## 💤 Bitácora Onírica (Últimos Sueños)

- **Día 1:** Soñó con `'viento_que_borra'` (arquetipo `sombra`).
  - *Insight:* El sombra busca equilibrio en pradera humeda compensando la energía de lo reprimido.
- **Día 2:** Soñó con `'espejo_que_miente'` (arquetipo `sombra`).
  - *Insight:* El sombra descansa en costa abierta mientras lo reprimido aguarda en el umbral.
- **Día 3:** Soñó con `'olor_a_descomposición'` (arquetipo `sombra`).
  - *Insight:* En pantano costero, el sombra negocia con lo reprimido en el único idioma posible: el silencio.

---

## 📖 Crónicas Episódicas (Memoria de Acontecimientos)

- Día 0: Cooperó de forma mutua y armónica con Chara. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Sophron. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Arete. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Elpis. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Kairos. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Bios. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Bios. Su lazo social se fortaleció.
- Día 0: Detectó y rechazó un intento de manipulación de Kore.
- Día 0: Detectó y rechazó un intento de manipulación de Kore.
- Día 0: Detectó y rechazó un intento de manipulación de Kore.
- Día 0: Detectó y rechazó un intento de manipulación de Kore.
- Día 0: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Soñó con 'viento_que_borra' (sombra). Insight: El sombra busca equilibrio en pradera humeda compensando la energía de lo reprimido.
- Día 1: El colapso de nuestra tribu. Solo quedamos 2.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Detectó y rechazó un intento de manipulación de Kore.
- Día 1: Manipuló con éxito y astucia a Iris para ceder recursos.
- Día 1: Manipuló con éxito y astucia a Iris para ceder recursos.
- Día 1: Manipuló con éxito y astucia a Iris para ceder recursos.
- Día 1: Manipuló con éxito y astucia a Iris para ceder recursos.
- Día 1: Manipuló con éxito y astucia a Iris para ceder recursos.
- Día 2: Manipuló con éxito y astucia a Iris para ceder recursos.
- Día 2: Soñó con 'espejo_que_miente' (sombra). Insight: El sombra descansa en costa abierta mientras lo reprimido aguarda en el umbral.
- Día 2: Manipuló con éxito y astucia a Iris para ceder recursos.
- Día 3: Soñó con 'olor_a_descomposición' (sombra). Insight: En pantano costero, el sombra negocia con lo reprimido en el único idioma posible: el silencio.
