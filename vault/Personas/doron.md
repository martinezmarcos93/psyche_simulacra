---
id: doron
nombre: Doron
rol: guardian
edad: 38
sexo: M
is_alive: True
posicion: [46, 27]
humor: 0.834
energia: 0.665
ansiedad: 0.333
needs:
  hambre: 0.064
  fatiga: 0.335
  sed: 0.375
  sociabilidad: 0.020
arquetipo_dominante: self
estado_conductual: competencia
arquetipos:
  self: 0.714
  persona: 0.500
  sombra: 0.350
  anima_animus: 0.400
  heroe: 0.522
  sabio: 0.400
  trickster: 0.250
  madre: 0.402
  padre: 0.401
  nino_divino: 0.300
  gobernante: 0.621
  rebelde: 0.300
---
# 👤 Persona: Doron

> **Rol:** `guardian` | **Edad:** 38 | **Sexo:** M | **Ubicación:** `(46, 27)`
> **Estado Vital:** **🟢 Conectado / Vivo**

---

## 📊 Estado Psicobiológico

### Capa Emocional
| Métrica | Visual | Valor |
| :--- | :---: | :---: |
| **Humor** | `[▓▓▓▓▓▓▓▓░░]` | 0.83 |
| **Energía** | `[▓▓▓▓▓▓▓░░░]` | 0.66 |
| **Ansiedad** | `[▓▓▓░░░░░░░]` | 0.33 |

### Necesidades Biológicas
| Necesidad | Visual | Valor | Estado |
| :--- | :---: | :---: | :--- |
| **Hambre** | `[▓░░░░░░░░░]` | 0.06 | 🟢 Saciado |
| **Sed** | `[▓▓▓▓░░░░░░]` | 0.38 | 🟢 Hidratado |
| **Fatiga** | `[▓▓▓░░░░░░░]` | 0.34 | 🟢 Descansado |
| **Sociabilidad** | `[░░░░░░░░░░]` | 0.02 | 🟢 Conectado |

---

## 🧠 Perfil Psicológico

### Capa Jungiana: Vector de Arquetipos
| Arquetipo | Fuerza | Barra Visual |
| :--- | :---: | :--- |
| **Self** | `0.714` | `▓▓▓▓▓▓▓▓▓░░░` |
| **Persona** | `0.500` | `▓▓▓▓▓▓░░░░░░` |
| **Sombra** | `0.350` | `▓▓▓▓░░░░░░░░` |
| **Anima animus** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Heroe** | `0.522` | `▓▓▓▓▓▓░░░░░░` |
| **Sabio** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Trickster** | `0.250` | `▓▓▓░░░░░░░░░` |
| **Madre** | `0.402` | `▓▓▓▓▓░░░░░░░` |
| **Padre** | `0.401` | `▓▓▓▓▓░░░░░░░` |
| **Nino divino** | `0.300` | `▓▓▓▓░░░░░░░░` |
| **Gobernante** | `0.621` | `▓▓▓▓▓▓▓░░░░░` |
| **Rebelde** | `0.300` | `▓▓▓▓░░░░░░░░` |

### Complejos Activos
| Complejo | Intensidad | Barra Visual | Estado |
| :--- | :---: | :--- | :---: |
| Abandono | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Inferioridad | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Poder | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Culpa | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Materno | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Trascendencia | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |

---

## 👥 Red Social e Interacciones

### Vínculos Emocionales
| Agente Relacionado | Vínculo | Visual Vínculo | Entrelazado |
| :--- | :---: | :---: | :---: |
| [[arete]] | `-0.30` | `▓▓▓▓░░░░░░` | Ninguno |
| [[bios]] | `-0.02` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[chara]] | `+0.34` | `▓▓▓▓▓▓▓░░░` | Ninguno |
| [[elpis]] | `-0.02` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[gigas]] | `-0.22` | `▓▓▓▓░░░░░░` | Ninguno |
| [[hados]] | `-0.04` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[iris]] | `-0.18` | `▓▓▓▓░░░░░░` | Ninguno |
| [[kore]] | `+0.02` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[leon]] | `-0.02` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[sophron]] | `-0.04` | `▓▓▓▓▓░░░░░` | Ninguno |

---

## 💤 Bitácora Onírica (Últimos Sueños)

- **Día 1:** Soñó con `'corona_que_duele'` (arquetipo `gobernante`).
  - *Insight:* En pradera humeda, el gobernante encuentra tregua provisional con lo reprimido.
- **Día 2:** Soñó con `'campo_interminable'` (arquetipo `gobernante`).
  - *Insight:* En pradera humeda, el gobernante encuentra tregua provisional con lo reprimido.
- **Día 3:** Soñó con `'río_tranquilo'` (arquetipo `gobernante`).
  - *Insight:* En valle fertil, el gobernante encuentra tregua provisional con lo reprimido.
- **Día 4:** Soñó con `'centro_sin_lugar'` (arquetipo `self`).
  - *Insight:* En colinas suaves, el self encuentra tregua provisional con lo reprimido.
- **Día 5:** Soñó con `'centro_sin_lugar'` (arquetipo `self`).
  - *Insight:* En valle fertil, el self encuentra tregua provisional con lo reprimido.

---

## 📖 Crónicas Episódicas (Memoria de Acontecimientos)

- Día 0: Se impuso competitivamente ante Elpis, absorbiendo sus recursos biológicos.
- Día 0: Se impuso competitivamente ante Hados, absorbiendo sus recursos biológicos.
- Día 0: Se impuso competitivamente ante Hados, absorbiendo sus recursos biológicos.
- Día 0: Se impuso competitivamente ante Leon, absorbiendo sus recursos biológicos.
- Día 0: Se impuso competitivamente ante Bios, absorbiendo sus recursos biológicos.
- Día 0: Se impuso competitivamente ante Sophron, absorbiendo sus recursos biológicos.
- Día 0: Se impuso competitivamente ante Sophron, absorbiendo sus recursos biológicos.
- Día 0: Se enfrentó en un choque violento y destructivo contra Gigas.
- Día 0: Detectó y rechazó un intento de manipulación de Arete.
- Día 0: Detectó y rechazó un intento de manipulación de Arete.
- Día 1: Soñó con 'corona_que_duele' (gobernante). Insight: En pradera humeda, el gobernante encuentra tregua provisional con lo reprimido.
- Día 1: Sufrió explotación y hostilidad de Iris, cediendo recursos biológicos.
- Día 2: Soñó con 'campo_interminable' (gobernante). Insight: En pradera humeda, el gobernante encuentra tregua provisional con lo reprimido.
- Día 2: Manipuló con éxito y astucia a Kore para ceder recursos.
- Día 2: Manipuló con éxito y astucia a Chara para ceder recursos.
- Día 3: Soñó con 'río_tranquilo' (gobernante). Insight: En valle fertil, el gobernante encuentra tregua provisional con lo reprimido.
- Día 3: Cooperó de forma mutua y armónica con Chara. Su lazo social se fortaleció.
- Día 3: Cooperó de forma mutua y armónica con Chara. Su lazo social se fortaleció.
- Día 3: Cooperó de forma mutua y armónica con Chara. Su lazo social se fortaleció.
- Día 3: Cooperó de forma mutua y armónica con Chara. Su lazo social se fortaleció.
- Día 4: Soñó con 'centro_sin_lugar' (self). Insight: En colinas suaves, el self encuentra tregua provisional con lo reprimido.
- Día 5: Soñó con 'centro_sin_lugar' (self). Insight: En valle fertil, el self encuentra tregua provisional con lo reprimido.
