---
id: bios
nombre: Bios
rol: cazador
edad: 34
sexo: M
is_alive: True
posicion: [40, 30]
humor: 0.802
energia: 0.659
ansiedad: 0.381
needs:
  hambre: 0.064
  fatiga: 0.335
  sed: 0.435
  sociabilidad: 0.020
arquetipo_dominante: heroe
estado_conductual: competencia
arquetipos:
  self: 0.647
  persona: 0.500
  sombra: 0.300
  anima_animus: 0.400
  heroe: 0.650
  sabio: 0.400
  trickster: 0.250
  madre: 0.403
  padre: 0.400
  nino_divino: 0.300
  gobernante: 0.400
  rebelde: 0.300
---
# 👤 Persona: Bios

> **Rol:** `cazador` | **Edad:** 34 | **Sexo:** M | **Ubicación:** `(40, 30)`
> **Estado Vital:** **🟢 Conectado / Vivo**

---

## 📊 Estado Psicobiológico

### Capa Emocional
| Métrica | Visual | Valor |
| :--- | :---: | :---: |
| **Humor** | `[▓▓▓▓▓▓▓▓░░]` | 0.80 |
| **Energía** | `[▓▓▓▓▓▓▓░░░]` | 0.66 |
| **Ansiedad** | `[▓▓▓▓░░░░░░]` | 0.38 |

### Necesidades Biológicas
| Necesidad | Visual | Valor | Estado |
| :--- | :---: | :---: | :--- |
| **Hambre** | `[▓░░░░░░░░░]` | 0.06 | 🟢 Saciado |
| **Sed** | `[▓▓▓▓░░░░░░]` | 0.44 | 🟢 Hidratado |
| **Fatiga** | `[▓▓▓░░░░░░░]` | 0.34 | 🟢 Descansado |
| **Sociabilidad** | `[░░░░░░░░░░]` | 0.02 | 🟢 Conectado |

---

## 🧠 Perfil Psicológico

### Capa Jungiana: Vector de Arquetipos
| Arquetipo | Fuerza | Barra Visual |
| :--- | :---: | :--- |
| **Self** | `0.647` | `▓▓▓▓▓▓▓▓░░░░` |
| **Persona** | `0.500` | `▓▓▓▓▓▓░░░░░░` |
| **Sombra** | `0.300` | `▓▓▓▓░░░░░░░░` |
| **Anima animus** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Heroe** | `0.650` | `▓▓▓▓▓▓▓▓░░░░` |
| **Sabio** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Trickster** | `0.250` | `▓▓▓░░░░░░░░░` |
| **Madre** | `0.403` | `▓▓▓▓▓░░░░░░░` |
| **Padre** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Nino divino** | `0.300` | `▓▓▓▓░░░░░░░░` |
| **Gobernante** | `0.400` | `▓▓▓▓▓░░░░░░░` |
| **Rebelde** | `0.300` | `▓▓▓▓░░░░░░░░` |

### Complejos Activos
| Complejo | Intensidad | Barra Visual | Estado |
| :--- | :---: | :--- | :---: |
| Abandono | `0.65` | `▓▓▓▓▓▓░░░░` | 💤 Inactivo |
| Inferioridad | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Poder | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Culpa | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Materno | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |
| Trascendencia | `0.30` | `▓▓▓░░░░░░░` | 💤 Inactivo |

---

## 👥 Red Social e Interacciones

### Vínculos Emocionales
| Agente Relacionado | Vínculo | Visual Vínculo | Entrelazado |
| :--- | :---: | :---: | :---: |
| [[doron]] | `-0.18` | `▓▓▓▓░░░░░░` | Ninguno |
| [[gigas]] | `-0.28` | `▓▓▓▓░░░░░░` | ⚛️ Entrelazado |
| [[kairos]] | `+0.08` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[leon]] | `+0.08` | `▓▓▓▓▓░░░░░` | Ninguno |
| [[moros]] | `+0.16` | `▓▓▓▓▓▓░░░░` | Ninguno |
| [[sophron]] | `-0.15` | `▓▓▓▓░░░░░░` | Ninguno |

---

## 💤 Bitácora Onírica (Últimos Sueños)

- **Día 1:** Soñó con `'victoria_vacía'` (arquetipo `heroe`).
  - *Insight:* El heroe y lo reprimido se toleran esta noche en valle fertil.
- **Día 2:** Soñó con `'espada_rota'` (arquetipo `heroe`).
  - *Insight:* El heroe descansa en valle fertil mientras lo reprimido aguarda en el umbral.
- **Día 3:** Soñó con `'abundancia_que_pudre'` (arquetipo `heroe`).
  - *Insight:* El heroe descansa en valle fertil mientras lo reprimido aguarda en el umbral.

---

## 📖 Crónicas Episódicas (Memoria de Acontecimientos)

- Día 0: Cooperó de forma mutua y armónica con Leon. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Gigas. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Gigas. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Kairos. Su lazo social se fortaleció.
- Día 0: Sufrió explotación y hostilidad de Doron, cediendo recursos biológicos.
- Día 0: Cooperó de forma mutua y armónica con Moros. Su lazo social se fortaleció.
- Día 0: Cooperó de forma mutua y armónica con Moros. Su lazo social se fortaleció.
- Día 0: Detectó y rechazó un intento de manipulación de Sophron.
- Día 0: Se enfrentó en un choque violento y destructivo contra Gigas.
- Día 0: Se enfrentó en un choque violento y destructivo contra Gigas.
- Día 1: Soñó con 'victoria_vacía' (heroe). Insight: El heroe y lo reprimido se toleran esta noche en valle fertil.
- Día 1: El colapso de nuestra tribu. Solo quedamos 2.
- Día 2: Soñó con 'espada_rota' (heroe). Insight: El heroe descansa en valle fertil mientras lo reprimido aguarda en el umbral.
- Día 3: Soñó con 'abundancia_que_pudre' (heroe). Insight: El heroe descansa en valle fertil mientras lo reprimido aguarda en el umbral.
